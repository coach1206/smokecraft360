...
-   **Craft Command Center (POS Flow)**: Complete end-to-end kiosk transaction flow. Entry Portal (`/` and `/entry`) with 4 animated tiles (Venue Access, Admin Control, Vendor Access, Demo Mode). PIN Login (`/pin-login`) with touch-optimized keypad and mock users (Owner:1111, Manager:2222, Staff:3333). POS Mode (`/pos`) with 3-panel layout: completed orders (left), product grid with category tabs (center), current order cart (right). Features 16 mock products across 4 categories (cigar/spirit/beer/food) with Unsplash images, live inventory tracking with low-stock warnings, reward trigger at $50+ (10% discount), checkout with confirmation animation. State managed via `PosContext` (React context). Files: `Entry.tsx`, `PinLogin.tsx`, `PosMode.tsx`, `PosContext.tsx`.
