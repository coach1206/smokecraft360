...
-   **Universal Swipe Experience Engine**: Tinder-style card swiping at `/experience/:type` (smoke/pour/brew/vape). Replaces 4 per-craft pages. Legacy routes auto-redirect. Includes `ExperiencePage` (card stack with drag physics) and `RevealPage` (3 revenue-scored recommendations). `craftThemes.ts` provides per-craft visual config.
