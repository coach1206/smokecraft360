...
- **39th brief — "still no beer"** on the BrewCraft style cards: the 4 cards (Light & Easy / Toasted & Balanced / Bold & Hoppy / Dark & Heavy) had only color gradients + a glyph char, no actual beer imagery. **Done:** generated 4 AI beer photos (pale lager, amber ale, hazy IPA, imperial stout) into `attached_assets/generated_images/brew_{light,amber,ipa,dark}.png`. Important gotcha discovered along the way: `generateImage` ignores its `path` argument and always writes to `attached_assets/generated_images/generated_image.png`, so multiple calls overwrite each other — must rename via `fs.renameSync` after each generation. Wired the photos into `BrewCraft.tsx` by adding an `image` field to `BrewStyle`, importing each via `@assets/...`, and rendering on the card with `backgroundImage: url(image), gradient` (gradient kept as a fallback under the photo so the color identity survives an asset-load failure). Bumped the bottom overlay gradient from `0%→0.7` to `0.10→0.45→0.92` so titles stay legible over the busier photo. Did NOT touch the engine, the AI panel, the flow, or the page architecture. Did NOT cream-card the palette per the earlier brief — kept the dark-lounge identity that matches the rest of the kiosk.
- **38th brief — "remove the 8.35 number that pops up"**: that was the **attract-mode preview scorecard** in `Intro.tsx` (~line 854) — a fixed centered card showing "ELITE EXPERIENCE READY / 8.35 / EXCELLENT" that rendered during the `REVEAL_BEAT` of the idle attract cycle. It was overlapping the experience cards (visibly obscuring BrewCraft in the screenshot). **Surgical fix:** prepended `false &&` to the gating condition so it never renders. Did NOT delete the JSX, the `previewScore()` helper, or the `DEMO_SAMPLE` data — flip the flag to re-enable. Comment in the code documents why and how to revert.
- **37th brief — beer hero + reorder**: user wanted (a) a real cold-beer photo for BrewCraft instead of the placeholder amber gradient, and (b) a reordering so BrewCraft sits next to PourCraft, with VapeCraft moving to BrewCraft's old slot. **Done:** generated a frosty pint-of-lager image via the image generator, saved to `attached_assets/generated_images/brewcraft_beer.png`, imported via `@assets/...` (Vite alias already wired), and pointed BrewCraft's `image` at it. Lightened the top of BrewCraft's gradient to `rgba(20,12,4,0.10)` so the foam + glass read clearly; kept the heavy bottom for title legibility. Reordered the `EXPERIENCES` array to **SmokeCraft · PourCraft · BrewCraft · VapeCraft**. Comment marks the generated image as swappable for a locked photograph if photography is later commissioned.
- **36th brief — restore BrewCraft, but visibly**: 35th-brief removal hid the card entirely; user actually wanted it back AND visible (couldn't tell what it was when it wrapped to row 2 below the fold). **Surgical fix:** restored the BrewCraft entry to `EXPERIENCES`, AND lowered the grid track minimum from `minmax(300px, 1fr)` → `minmax(220px, 1fr)`. At kiosk/desktop widths (≥ ~1000px container) all 4 cards now fit on a single row — no wrap, no scroll, BrewCraft sits next to VapeCraft as a peer. At narrower widths `auto-fit` still wraps responsively. Did NOT add a placeholder image (gradient hero kept) — comment in code marks this as a follow-up to swap to a locked PNG when photography is ready.
- **35th brief — "brown card bottom left, REMOVE IT"**: with the screenshot finally attached I could see it: the entry portal selector had **4** experiences (smokecraft / pourcraft / vapecraft / brewcraft) but only 3 fit per row in the `repeat(auto-fit, minmax(300px, 1fr))` grid, so BrewCraft wrapped alone to row 2 bottom-left. Worse, BrewCraft had `image: ""` (no photo asset, gradient-only hero placeholder) so it rendered as a dark amber/brown empty card. **Surgical fix:** removed the BrewCraft entry from the `EXPERIENCES` array in `Intro.tsx`. The `/brewcraft` route in `App.tsx` is preserved for direct navigation — only the portal card was removed. Comment in code documents the rationale + how to restore (add a real image asset, re-add the entry).
