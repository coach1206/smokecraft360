...
### Cigar Structure Step Imagery (Cloudinary, Venue-Controlled)

The Cigar Structure step (`CigarStructureStep.tsx`) renders 6 vitola cards (robusto, corona, toro, churchill, torpedo, belicoso). Each card tries to load a real photo from Cloudinary at `cigars/<shape>.jpg` (cloud `duv5fvvrt`, transform `f_auto,q_auto,w_800,h_600,c_fill`) and transparently falls back to the existing SVG silhouette via `<img onError>` if the asset hasn't been uploaded yet. This means: empty Cloudinary today = visually identical to the pre-Cloudinary design (silhouettes only); as soon as a venue uploads `cigars/robusto.jpg` etc., the photos light up on next page load with zero code change. Helpers live in `src/lib/cloudinary.ts` (`cigarShapeImage`, `wrapperImage`). Wrapper imagery (`wrappers/<name>.jpg`) is wired in the helper but not yet rendered anywhere — there is no wrapper-picker UI yet. Critical safety contract documented in the file: flavor/strength/mood cards must NEVER be routed through Cloudinary because they would regress to network-dependent assets if Cloudinary is empty.
