...
function QRPattern({ data }: { data: string }) {
  // Deterministic visual QR-like grid from data string
  const hash = data.split("").reduce((a, c) => (a * 31 + c.charCodeAt(0)) & 0xffffffff, 0);
  const cells = Array.from({ length: 25 }, (_, i) => (hash >> (i % 32)) & 1);
  return (
