...
export default function EATConsole({ defaultTab: _defaultTab }: { defaultTab?: string }) {
  ...
  const handlePinConfirm = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json() as { ok?: boolean; error?: string; tier?: string; token?: string };
      if (data.ok) {
        const stored  = localStorage.getItem("axiom_token") ?? data.token ?? "";
        ...
        if (pendingAction === "open_tab") {
          void fetch("/api/tabs/open", {
            method: "POST", headers,
            body: JSON.stringify({ guestName: "Walk-in Guest", venueId }),
          }).then(r => r.ok ? r.json() : null).catch(() => {});
        }
        if (pendingAction === "close_tab" && activeTabs[0]) {
          const tabId = activeTabs[0].id;
          void fetch(`/api/tabs/${tabId}/close`, { method: "POST", headers, body: JSON.stringify({}) })
            .then(r => r.ok ? r.json() : null)
            .then(() => setActiveTabs(prev => prev.filter(t => t.id !== tabId)))
            .catch(() => {});
        }
        if (pendingAction === "void_item" && activeTabs[0]) {
          const tabId = activeTabs[0].id;
          void fetch(`/api/tabs/${tabId}/void`, { method: "POST", headers, body: JSON.stringify({}) })
            .then(r => r.ok ? r.json() : null)
            .then(() => setActiveTabs(prev => prev.filter(t => t.id !== tabId)))
            .catch(() => {});
        }
  ...
  }, [pinEntry, pendingAction, pinSubmitting, activeTabs]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/tabs/venue/${encodeURIComponent(venueId)}`, {
      ...
      .then((d: unknown) => {
        ...
        if (rows && rows.length > 0) setActiveTabs(rows);
      })
      .catch(() => {
        setActiveTabs([
      ...
      <header style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ color: "rgba(212,175,55,0.28)", fontSize: 18, letterSpacing: "0.14em" }}>// SMOKECRAFT 360</span>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          {lowStock.length > 0 && (
            <div style={{ display: "flex", alignItems: "center", gap: 7, padding: "7px 14px",
              ...
              <span style={{ color: AMBER_H, fontSize: 18, fontWeight: 800, letterSpacing: "0.16em" }}>
      ...
      <main style={{
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, overflow: "auto" }}>
          ...
          <Card highlight style={{ padding: "18px 22px" }}>
            <div style={{ fontSize: 18, letterSpacing: "0.22em", color: "rgba(212,175,55,0.48)", marginBottom: 14, textTransform: "uppercase" }}>SHIFT METRICS</div>
          ...
          <Card>
            <div style={{ padding: "13px 18px", borderBottom: `1px solid rgba(212,175,55,0.07)`,
              ...
              <span style={{ fontSize: 18, letterSpacing: "0.22em", color: "rgba(212,175,55,0.48)", textTransform: "uppercase" }}>ACTIVE GUEST TABS</span>
              <span style={{ fontSize: 18, color: TEAL, fontWeight: 800, letterSpacing: "0.12em" }}>{activeTabs.length} OPEN</span>
            </div>
            <div style={{ overflow: "auto", maxHeight: 218 }}>
              {activeTabs.length === 0
                ? <div style={{ padding: "24px", textAlign: "center", color: "rgba(255,255,255,0.16)", fontSize: 18, letterSpacing: "0.14em" }}>NO ACTIVE TABS</div>
                : activeTabs.map(tab => (
                  <div key={tab.id} style={{
                    ...
                    <div>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
                        <span style={{ fontSize: 18, fontWeight: 700, color: "rgba(255,255,255,0.84)" }}>{tab.name}</span>
          ...
          <Card style={{ padding: "15px 16px" }}>
            <div style={{ fontSize: 18, letterSpacing: "0.22em", color: "rgba(212,175,55,0.55)", marginBottom: 11, textTransform: "uppercase", fontWeight: 800 }}>QUICK ACTIONS · PIN REQUIRED</div>
          ...
          <Card style={{ padding: "15px 18px" }}>
            <div style={{ fontSize: 18, letterSpacing: "0.22em", color: "rgba(212,175,55,0.48)", marginBottom: 13, textTransform: "uppercase" }}>MILESTONE TRACKER</div>
          ...
          <Card>
            <div style={{ padding: "13px 18px", borderBottom: `1px solid rgba(212,175,55,0.12)`,
              display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: 18, letterSpacing: "0.22em", color: "rgba(212,175,55,0.55)", textTransform: "uppercase", fontWeight: 800 }}>RECENT ACTIVITY</span>
              <span style={{ fontSize: 18, color: TEAL, fontWeight: 800, letterSpacing: "0.12em" }}>LAST {Math.min(activeTabs.length, 10)}</span>
            </div>
            <div style={{ overflow: "auto", maxHeight: 240 }}>
              {activeTabs.slice(0, 10).map(tab => (
                <div key={tab.id} style={{ padding: "11px 18px", borderBottom: `1px solid rgba(255,255,255,0.03)`,
                  display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    <div style={{ fontSize: 18, fontWeight: 700, color: "rgba(255,255,255,0.84)", marginBottom: 2 }}>{tab.name}</div>
                    <div style={{ fontSize: 18, color: "rgba(255,255,255,0.30)", letterSpacing: "0.10em" }}>
                      TABLE {tab.tableNumber} · {tab.itemCount ?? "—"} ITEMS · {tab.openedAt ?? "—"}
                    </div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: 22, fontWeight: 900, color: GOLD, lineHeight: 1 }}>${tab.total.toLocaleString()}</div>
                    <div style={{ fontSize: 18, fontWeight: 800, letterSpacing: "0.12em", textTransform: "uppercase", marginTop: 3, color: loyaltyColor(tab.loyalty ?? "") }}>
                      {tab.loyalty ?? "—"}
                    </div>
                  </div>
                </div>
        ...
        {pendingAction && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.92, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <div style={{ fontSize: 18, letterSpacing: "0.26em", color: `${GOLD}88`, marginBottom: 8, textTransform: "uppercase", fontWeight: 700 }}>STAFF AUTHORIZATION</div>
              ...
              />
              {pinError && (
                <div style={{ fontSize: 18, color: RED_H, marginBottom: 14, letterSpacing: "0.12em", fontWeight: 700 }}>{pinError}</div>
              )}
              <div style={{ display: "flex", gap: 10 }}>
                <button
                  ...
                  style={{ flex: 1, padding: "14px", background: "rgba(255,255,255,0.04)",
                    ...
                    color: "rgba(255,255,255,0.50)", cursor: "pointer", fontSize: 18,
