/**
 * S2_TerroirMatrix — THE ALCHEMIST · THE CRAFT
...
// ── Mentor data per origin country ───────────────────────────────────────────
...
type Step = "intro" | "wrapper" | "anatomy" | "balancing" | "mentor_challenge" | "flavor_wheel" | "complete";

interface MeritFeedback { amount: number; type: "merit" | "points" }

// ── Component ─────────────────────────────────────────────────────────────────
export function S2_TerroirMatrix() {
  const { profile, updateProfile, setPhase, addPoints, applyPenalty } = useNoveeGuest();
  ...
  const [challengeDone, setChallDone]   = useState(false);
  ...
  function calcBlendScore(): { merit: number; label: string } {
    ...
    const balanced = total >= 92 && total <= 108;
    if (balanced && vOk && sOk && lOk) return { merit: calcMeritDelta("blend_perfect", profile.difficultyTier), label: "PERFECT BALANCE" };
    if (balanced && (vOk || sOk)) return { merit: 2, label: "GOOD BALANCE" };
    return { merit: calcMeritDelta("blend_poor", profile.difficultyTier), label: "POOR BALANCE" };
  }
  ...
  function handleFinalizeBlend() {
    ...
    if (score.merit >= 0) {
      addPoints(score.merit > 0 ? 15 : 5);
      setMeritFb({ amount: score.merit, type: "merit" });
    } else {
      applyPenalty(Math.abs(score.merit));
      setMeritFb({ amount: score.merit, type: "merit" });
    }
    hapticMilestone();
    trackEvent({ type: "blend_saved", phase: "s2_balancing", data: { volado, seco, ligero, score: score.label }, sessionType: profile.sessionType, timestamp: Date.now() });
    go("mentor_challenge");
  }
  ...
  return (
    <div style={{ position: "absolute", inset: 0, overflow: "hidden", background: "#050505", color: "#F0E8D4", fontFamily: "'Inter', sans-serif" }}>
      ...
      <AnimatePresence>
        {showLigeroWarn && (
          <motion.div
            ...
            style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(0,0,0,0.92)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 48 }}
          >
            <motion.div initial={{ scale: 0.8, y: 40 }} animate={{ scale: 1, y: 0 }} transition={PT}
              ...
              <div style={{ fontSize: 40, fontWeight: 900, fontFamily: "'Cormorant Garamond', serif", color: "#F0E8D4", marginBottom: 24, lineHeight: 1.1 }}>
                {mentorData.name}
              </div>
              ...
              <motion.button
                onClick={() => setLigeroWarn(false)} whileTap={{ scale: 0.96 }}
      ...
      <AnimatePresence>
        {meritFeedback && (
          <XPFeedback amount={meritFeedback.amount} type={meritFeedback.type} onComplete={() => setMeritFb(null)} />
        )}
      </AnimatePresence>
      ...
      {(["intro","wrapper","anatomy","balancing","mentor_challenge","flavor_wheel"] as Step[]).map((s, i) => (
        <div key={s} style={{
          position: "absolute", top: 16, left: `calc(50% - 90px + ${i * 36}px)`,
      ...
      <AnimatePresence mode="wait">
        ...
        {step === "intro" && (
          <motion.div key="intro" variants={PV} initial="enter" animate="active" exit="exit" transition={PT}
            ...
            <div style={{ fontSize: 13, letterSpacing: "0.35em", color: GOLD, marginBottom: 16, fontWeight: 800, textTransform: "uppercase" }}>The Alchemist</div>
            ...
            <p style={{ fontSize: 24, opacity: 0.65, marginBottom: 48, maxWidth: 540, lineHeight: 1.5 }}>
              Transform raw terroir into a masterwork of construction. Your mentors await.
            </p>
        ...
        {step === "wrapper" && (
          <motion.div key="wrapper" variants={PV} initial="enter" animate="active" exit="exit" transition={PT}
            ...
            <h2 style={{ fontSize: 42, fontWeight: 400, fontFamily: "'Cormorant Garamond', serif", marginBottom: 8, color: "#F0E8D4" }}>Select Your Wrapper</h2>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 16 }}>
              {WRAPPERS.map(w => (
                <motion.div key={w.id} onClick={() => handleWrapperSelect(w)} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
                  style={{
                    background: wrapper === w.id ? `${w.color}22` : "rgba(255,255,255,0.03)",
                    border: wrapper === w.id ? `2px solid ${w.color}` : "1px solid rgba(255,255,255,0.08)",
                    borderRadius: 12, overflow: "hidden", cursor: "pointer",
                    boxShadow: wrapper === w.id ? `0 0 20px ${w.color}44` : "none",
                    transition: "all 0.2s",
                  }}>
                  {/* color swatch */}
                  ...
                  <div style={{ padding: "16px 20px" }}>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                      {w.notes.map(n => <span key={n} style={{ fontSize: 12, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 4, padding: "3px 8px", color: "#F0E8D4aa" }}>{n}</span>)}
                    </div>
                  </div>
                </motion.div>
              ))}
        ...
        {step === "anatomy" && (
          <motion.div key="anatomy" variants={PV} initial="enter" animate="active" exit="exit" transition={PT}
            ...
            <p style={{ fontSize: 24, opacity: 0.55, marginBottom: 36 }}>Every great cigar is a three-layer composition. Tap each layer to understand its role.</p>

            {[
              { label: "WRAPPER", sub: "Outer Leaf", desc: "Defines aesthetic appeal, aroma, and the first third of the smoking experience. Must be pristine and oil-rich.", color: GOLD },
              ...
              <motion.div key={layer.label} whileHover={{ x: 4 }}
                style={{ display: "flex", gap: 20, marginBottom: 20, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 12, padding: 24, borderLeft: `3px solid ${layer.color}` }}>
                <div>
        ...
        {step === "balancing" && (
          <motion.div key="balancing" variants={PV} initial="enter" animate="active" exit="exit" transition={PT}
            ...
            <p style={{ fontSize: 24, opacity: 0.55, marginBottom: 32 }}>Balance combustion, aroma, and strength. Your mentor's challenge awaits after submission.</p>
            ...
            <div style={{ background: `${mentorData.color}15`, border: `1px solid ${mentorData.color}44`, borderRadius: 12, padding: "20px 24px", marginBottom: 32 }}>
              <div style={{ fontSize: 12, letterSpacing: "0.2em", color: mentorData.color, marginBottom: 4, fontWeight: 800 }}>{mentorData.name.toUpperCase()} — BLEND TARGET</div>
            ...
            {ligero > 40 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                ...
                MENTOR WARNING — Ligero exceeds 40%. Draw and burn stability at risk.
              </motion.div>
            )}
        ...
        {step === "mentor_challenge" && (
          <motion.div key="mentor_challenge" variants={PV} initial="enter" animate="active" exit="exit" transition={PT}
            ...
            <motion.div
              initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15, ...PT }}
              style={{
                background: `radial-gradient(ellipse at top, ${mentorData.color}22 0%, rgba(5,5,5,0.95) 70%)`,
                border: `1px solid ${mentorData.color}66`,
                borderRadius: 20, padding: "40px 48px", maxWidth: 600, width: "100%",
                backdropFilter: "blur(20px)",
                boxShadow: `0 0 60px ${mentorData.color}33, inset 0 1px 0 ${mentorData.color}44`,
                marginBottom: 40,
              }}>
              ...
              <div style={{ fontSize: 16, color: `${mentorData.color}cc`, marginBottom: 28, letterSpacing: "0.05em" }}>{mentorData.profile}</div>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.45 }}
              ...
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: 13, letterSpacing: "0.2em", color: GOLD, marginBottom: 4 }}>BLEND BALANCE</div>
                <div style={{ fontSize: 36, fontWeight: 900, color: "#F0E8D4" }}>{total}%</div>
              </div>
              <div style={{ textAlign: "center" }}>
            ...
            <motion.button onClick={() => { setChallDone(true); go("flavor_wheel"); }} whileTap={{ scale: 0.96 }}
              style={{ padding: "22px 48px", background: GOLD, color: "#000", border: "none", borderRadius: 4, fontSize: 20, fontWeight: 900, cursor: "pointer", letterSpacing: "0.1em" }}>
          ...
          <motion.div key="flavor_wheel" variants={PV} initial="enter" animate="active" exit="exit" transition={PT}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 40 }}>
              {FLAVOR_NODES.map(f => {
                ...
                return (
                  <motion.div key={f}
                    onClick={() => setNotes(prev => prev.includes(f) ? prev.filter(x => x !== f) : [...prev, f])}
                    whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}
                    style={{
                      padding: "22px 12px", border: `1px solid ${sel ? GOLD : "rgba(255,255,255,0.12)"}`,
                      borderRadius: 10, textAlign: "center", cursor: "pointer",
                      background: sel ? `${GOLD}18` : "transparent",
                      boxShadow: sel ? `0 0 12px ${GOLD}33` : "none",
                      fontSize: 20, fontWeight: sel ? 800 : 400, color: sel ? GOLD : "#F0E8D4aa",
                      transition: "all 0.2s",
                    }}>
                    {f}
                  </motion.div>
