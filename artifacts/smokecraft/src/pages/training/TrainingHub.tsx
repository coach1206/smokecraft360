 ...
 *
 * Shows the simulated Vault Cigar Lounge with live events,
 * a KPI strip, and six mode-selection cards.
 */
...
import {
  ...
  Award, Play, Zap, ArrowLeft, Activity,
...
const MODES = [
  {
    ...
    subtitle: "Role-based onboarding",
    desc: "Interactive walkthroughs for every venue role — from bartender to owner.",
    icon: Users, color: T.gold, route: "/training/employee",
  },
  {
    id: "investor",
    title: "Investor Demo",
    subtitle: "Cinematic auto-demo",
    desc: "A guided journey through AI intelligence, revenue automation, and scale.",
    icon: TrendingUp, color: T.green, route: "/training/investor",
  },
  {
    id: "sales",
    title: "Sales Presentation",
    subtitle: "Venue-owner showcase",
    desc: "Walk venue owners through increased engagement, spend, and retention.",
    icon: DollarSign, color: T.blue, route: "/training/sales",
  },
  {
    id: "walkthrough",
    title: "Venue Walkthrough",
    subtitle: "10-step setup flow",
    desc: "From first launch to full operation — guided touchscreen walkthrough.",
    icon: Map, color: T.purple, route: "/training/walkthrough",
  },
  {
    id: "scenarios",
    title: "Scenario Missions",
    subtitle: "8 training scenarios",
    desc: "Real-world simulations with scoring, coaching, and Maxwell guidance.",
    icon: BookOpen, color: T.amber, route: "/training/scenarios",
  },
  {
    id: "certifications",
    title: "Certifications",
    subtitle: "Training records",
    desc: "View earned certifications and track completion across all roles.",
    icon: Award, color: "#06b6d4", route: "/training/certifications",
  },
];
...
function KpiStrip() {
  const [vals, setVals] = useState(DEMO_KPIS);

  useEffect(() => {
    const t = setInterval(() => {
      setVals((v) => ({
        ...v,
        revenueTonight: v.revenueTonight + Math.floor(Math.random() * 20),
        aiConfidence:   Math.min(100, v.aiConfidence + (Math.random() > 0.5 ? 1 : 0)),
        tabsPaid:       v.tabsPaid + (Math.random() > 0.8 ? 1 : 0),
    ...
    <div style={{ display: "flex", gap: 10, overflowX: "auto", paddingBottom: 2 }}>
      {kpis.map(({ label, value, color }) => (
        ...
        >
          ...
          <div style={{ fontSize: 9, color: T.muted, textTransform: "uppercase", letterSpacing: "0.12em", marginTop: 2 }}>
            {label}
          </div>
        </motion.div>
      ))}
...
function LiveFeed() {
  const [events, setEvents] = useState(LIVE_EVENTS.slice(0, 6));
          ...
          return (
            ...
            >
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <span style={{ fontSize: 8, color, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", flexShrink: 0 }}>
                  {ev.type}
                </span>
    ...
    >
      <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 9, color: mode.color, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 6 }}>
            {mode.subtitle}
          </div>
        ...
        <motion.div
          ...
          style={{ color: mode.color, paddingTop: 2 }}
        >
          <Play size={12} />
        </motion.div>
      </div>
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {MODES.map((mode, i) => (
                ...
                >
                  <ModeCard mode={mode} onClick={() => navigate(mode.route)} />
                </motion.div>
              ))}
            </div>
          </motion.div>
