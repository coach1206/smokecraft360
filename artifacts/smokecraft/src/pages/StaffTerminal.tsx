...
function StaffRosterModal({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    api.staffRoster().then(rows => { if (rows.length > 0) setStaff(rows); });
  }, []);
  ...
  const addMember = async () => {
    ...
    const local: StaffMember = {
      staffId: `local_${Date.now()}`, staffName: newName.trim(),
        ...
        <div style={{ flex:1, overflowY:"auto" }}>
          {staff.map((member, idx) => (
            <div key={member.staffId} style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 22px",
              borderBottom:idx < staff.length - 1 ? `1px solid ${C.chrome}` : "none",
              ...
              <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                ...
                <motion.div whileTap={{scale:0.92}} onClick={() => toggleActive(member)}
                  ...
                  <motion.div layout style={{ width:20, height:20, borderRadius:"50%",
                    background: member.isActive ? "#FFF" : C.chrome }} />
                </motion.div>
              </div>
            </div>
          ))}
        ...
        <div style={{ padding:"16px 22px", borderTop:`1px solid ${C.chrome}`, flexShrink:0 }}>
          ...
          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
            ...
            <motion.button whileTap={{scale:0.96}} {...T}
              onTouchStart={e => { T.onTouchStart(e); addMember(); }}
              onClick={addMember}
