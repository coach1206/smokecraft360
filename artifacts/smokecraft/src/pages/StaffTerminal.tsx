...
// ── Default reservation queue (seed data — API overlay on auth success) ────────
...
const SKU_TO_PRODUCT: Record<string, string> = {
  sku_rp_1992:  "cigar-005",   // Rocky Patel Vintage 1992
  sku_bt_bourb: "alc-001",     // Buffalo Trace Bourbon
  sku_rm_xo:    "alc-001",     // Remy Martin XO → nearest spirit
  sku_p64:      "cigar-002",   // Padron 1964 Anniversary
  sku_mac12:    "alc-004",     // Macallan → Lagavulin 16 (closest scotch)
  sku_af_gr:    "cigar-001",   // Arturo Fuente Hemingway
};
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        ...
        // Advance cursor to most recent event
        ...
        events.forEach(evt => {
          ...
          const items = (payload["items"] as Array<{
            category?: string; qty?: number; quantity?: number; name?: string;
          }>) ?? [];
          items.forEach(item => {
            const qty = Math.max(1, Number(item.qty ?? item.quantity ?? 1));
            const cat = String(item.category ?? "").toLowerCase();
            if (cat.includes("cigar") || cat.includes("smoke")) {
              cigarsConsumed += qty;
            } else if (cat.includes("spirit") || cat.includes("liquor") || cat.includes("whiskey") || cat.includes("bourbon")) {
              spiritsDepleted.push(String(item.name ?? "Spirit"));
            }
          });
        });

        if (cigarsConsumed > 0 || spiritsDepleted.length > 0) {
          cbRef.current({ provider, eventType, cigarsConsumed, spiritsDepleted });
        }
      } catch { /* kiosk resilience — silent */ }
    };
...
// ── Header ────────────────────────────────────────────────────────────────────
function Header({ lastSync, syncAge }: { lastSync: Date; syncAge: number }) {
  ...
  const adj = useCallback((id: string, d: number) => {
    setItems(prev => {
      const next = prev.map(i => {
        ...
        // Track net mutation for stock deduction
            ...
            {displayProviders.map(prov => {
              ...
              return (
                <motion.button key={prov.provider}
                  ...
                  <div style={{ fontSize:14, color: isSyncing ? col : col, fontWeight:700, textAlign:"center", lineHeight:1.1 }}>{name}</div>
      ...
      <motion.div initial={{ scale:0.94, y:20 }} animate={{ scale:1, y:0 }}
        ...
        <div style={{ padding:"20px 24px 16px", borderBottom:`1px solid ${MT.border}`,
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            <div>
              <div style={{ fontSize:10, fontFamily:MT.mono, color:MT.gold, letterSpacing:"0.3em", marginBottom:4 }}>
                ENTERPRISE MULTI-TENANT
              </div>
              <div style={{ fontSize:22, fontWeight:900, color:MT.white, letterSpacing:"0.04em" }}>
                TENANT PROVISIONING MATRIX
              </div>
            </div>
            <motion.button whileTap={{ scale:0.9 }} onClick={onClose}
          ...
          <div style={{ marginTop:8, fontSize:11, fontFamily:MT.mono, color:MT.muted, letterSpacing:"0.14em" }}>
            TENANT LEASE: <span style={{ color:MT.gold }}>TENANT_PROFOUND_001</span>
            {" "}·{" "}
            <span style={{ color:MT.green }}>● ACTIVE</span>
          </div>
        </div>
          ...
          {status !== "done" ? (
            <>
              <div style={{ marginBottom:16 }}>
                <div style={{ fontSize:10, fontFamily:MT.mono, color:MT.muted, letterSpacing:"0.22em", marginBottom:8 }}>
                  VENUE NAME *
                </div>
              ...
              <motion.button whileTap={{ scale:0.97 }}
                ...
                style={{ width:"100%", height:52, marginBottom:20, display:"flex", alignItems:"center",
                  ...
                  border:`1px solid ${hydrate ? MT.gold : MT.border}`, borderRadius:10, cursor:"pointer",
                  transition:"all 0.2s" }}>
                ...
                <div style={{ textAlign:"left" }}>
                  <div style={{ fontSize:13, fontWeight:800, color:MT.white, letterSpacing:"0.04em" }}>
                    PROVISION UNIVERSAL LUXURY ASSET LIBRARY
                  </div>
                  <div style={{ fontSize:10, color:MT.muted, fontFamily:MT.mono, marginTop:2 }}>
                    Hydrates 24 standard Cigar & Spirit SKU templates instantly
                  </div>
                </div>
              </motion.button>
              ...
              <motion.button whileTap={{ scale:0.97 }}
                onClick={provision} disabled={!venueName.trim() || status === "busy"}
            ...
            <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }}
              ...
              <div style={{ fontSize:18, fontWeight:900, color:MT.gold, marginBottom:8 }}>
              ...
              <div style={{ ...mtPanel({ padding:"14px 18px", marginBottom:16, textAlign:"left" }) }}>
                ...
                {(result?.recordsHydrated ?? 0) > 0 && (
                  <div style={{ marginTop:10, fontSize:11, color:MT.green, fontFamily:MT.mono }}>
                    ✓ {result?.recordsHydrated} SKU templates hydrated
                  </div>
                )}
              </div>
              <motion.button whileTap={{ scale:0.97 }} onClick={onClose}
...
function HaReplicaPanel({ onClose }: { onClose: () => void }) {
  ...
  const runFailover = async () => {
    ...
    if (r?.success) {
      setNodes(r.currentClusterTopology);
      setMessage(r.message);
      setFailoverState("swapped");
    } else {
  ...
  const nodeAccent = (n: ClusterNode) =>
    n.status === "ONLINE" ? (isPrimary(n) ? MT.green : MT.amber) :
    n.status === "STANDBY" ? MT.muted : MT.red;
      ...
      <motion.div initial={{ scale:0.94, y:20 }} animate={{ scale:1, y:0 }}
        ...
        <div style={{ padding:"20px 24px 16px", borderBottom:`1px solid ${MT.border}`,
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            <div>
              <div style={{ fontSize:10, fontFamily:MT.mono, color:MT.gold, letterSpacing:"0.3em", marginBottom:4 }}>
                DEVELOPER DASHBOARD — HA CLUSTER
              </div>
              <div style={{ fontSize:22, fontWeight:900, color:MT.white, letterSpacing:"0.04em" }}>
                HA REPLICA NODE STATUS ENGINE
              </div>
        ...
        <div style={{ padding:"24px" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:12, marginBottom:20 }}>
            {nodes.map(node => {
              ...
              const syncDeltaMs = node.role === "READ_REPLICA_MIRROR"
                ? Math.round(Date.now() - new Date(node.lastSyncAt).getTime())
                : null;
              return (
                <div key={node.nodeId} style={{ ...mtPanel({
                  padding:"16px 18px", border:`1px solid ${accent}44`,
                  boxShadow:`0 0 18px ${accent}11` }) }}>
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:8 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                      <motion.div
                        animate={{ opacity: node.status === "STANDBY" ? 0.4 : [1, 0.3, 1] }}
                    ...
                    <div style={{ fontSize:9, fontFamily:MT.mono, color:MT.muted, letterSpacing:"0.14em" }}>
                      {node.latencyMs}ms
                    </div>
                  </div>
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                    ...
                    <div style={{ fontSize:9, fontFamily:MT.mono, color:MT.muted }}>
                      {node.status}
                    </div>
                  </div>
                  {syncDeltaMs !== null && (
                    <div style={{ marginTop:6, display:"flex", alignItems:"center", gap:8 }}>
                      ...
                      <div style={{ fontSize:9, fontFamily:MT.mono, color:accent, flexShrink:0 }}>
                        SYNC Δ {syncDeltaMs}ms
                      </div>
                    </div>
                  )}
                </div>
            ...
            {nodes.length === 0 && (
              <div style={{ textAlign:"center", color:MT.muted, fontFamily:MT.mono, fontSize:11, padding:"20px 0" }}>
                POLLING CLUSTER HEALTH...
              </div>
            )}
          </div>
          ...
          <motion.button whileTap={{ scale:0.97 }} onClick={runFailover}
            disabled={failoverState === "testing"}
            style={{ width:"100%", height:58, borderRadius:10, cursor:"pointer",
              background: failoverState === "swapped"
                ? `linear-gradient(135deg,${MT.green},#16A34A)` : `linear-gradient(135deg,${MT.red},#991B1B)`,
            ...
            {failoverState === "testing"
              ...
              : failoverState === "swapped"
              ? "REPLICA PROMOTED — RE-TEST FAILOVER"
              : "FORCE REPLICA HOT-SWAP TEST"}
          </motion.button>
        </div>
      </motion.div>
    </motion.div>
      ...
      {visible && cur && (
        <motion.div initial={{x:"100%",opacity:0}} animate={{x:0,opacity:1}} exit={{x:"100%",opacity:0}}
          ...
          <motion.button whileTap={{scale:0.9}} onClick={() => setVisible(false)}
            style={{ marginLeft:"auto", width:22, height:22, borderRadius:"50%", background:"rgba(255,255,255,0.07)",
              border:`1px solid ${C.chrome}`, color:C.muted, cursor:"pointer", fontSize:12,
              display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
...
function PosMenuMapperModal({ onClose }: { onClose: () => void }) {
  ...
  const renderSection = (label: string, items: CatalogEntry[]) => (
    <div style={{ marginBottom: 18 }}>
      <div style={{ fontSize: 10, fontFamily: C.mono, color: C.amber, letterSpacing: "0.24em", marginBottom: 8, paddingLeft: 4 }}>
        {label}
      </div>
      {items.map(p => (
        <div key={p.productId + p.sku} style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 14px",
          ...
          <input
            value={mappings[p.productId] ?? ""}
            onChange={e => setMappings(prev => ({ ...prev, [p.productId]: e.target.value }))}
            placeholder="POS Item ID / Token"
            style={{ width: 180, height: 38, background: "rgba(255,255,255,0.06)", border:`1px solid ${C.chrome}`,
              borderRadius: 6, color: C.white, fontSize: 14, fontFamily: C.mono, padding: "0 10px",
        ...
        <div style={{ padding:"16px 22px", borderBottom:`1px solid ${C.chrome}`, display:"flex", justifyContent:"space-between", alignItems:"center", flexShrink:0 }}>
          ...
          <motion.button whileTap={{scale:0.9}} onClick={onClose} {...T}
            style={{ width:36, height:36, borderRadius:9, background:"rgba(255,255,255,0.05)", border:`1px solid ${C.chrome}`,
              color:C.muted, cursor:"pointer", fontSize:18, display:"flex", alignItems:"center", justifyContent:"center" }}>
            <Icon d={P.close2} size={18} color={C.muted} />
          </motion.button>
        </div>
    ...
    <motion.div key="roster" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
      ...
      <motion.div initial={{scale:0.93,y:24}} animate={{scale:1,y:0}} exit={{scale:0.93,y:24}}
        ...
        <div style={{ flex:1, display:"flex", overflow:"hidden" }}>
          ...
          {/* Audit Telemetry Ticker + Proximity Blast */}
        ...
        <div style={{ padding:"14px 22px", borderTop:`1px solid ${C.chrome}`, flexShrink:0 }}>
          ...
          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
            ...
            <input value={newPin} onChange={e => setNewPin(e.target.value.replace(/\D/g,"").slice(0,4))} placeholder="4-digit PIN"
              type="password"
...
export default function StaffTerminal({ onBack: onBackProp }: { onBack?: () => void } = {}) {
  ...
  const refreshInventory = useCallback(async () => {
    ...
    // Seed humidor: sum of cigar qty in DB (fall back to local if all-zero seed)
    const dbPuros = cigars.reduce((s, p) => s + (p.qty ?? 0), 0);

    // Build low-stock spirit alerts: any spirit with qty > 0 but low (or par set)
    ...
    setVenueState(prev => ({
      ...
      telemetry: {
        ...
        humidor: {
          ...
          // Use DB count if non-zero, otherwise keep local simulation value
          purosRemaining: dbPuros > 0 ? dbPuros : prev.telemetry.humidor.purosRemaining,
        },
        bar: {
          ...prev.telemetry.bar,
          // Merge DB alerts with our seeded local alert
          lowStockAlerts: lowStockSpirits.length > 0 ? lowStockSpirits : prev.telemetry.bar.lowStockAlerts,
        },
      },
    }));
  }, []);
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      refreshInventory();
      // Simulate natural humidor climate drift
      setVenueState(prev => ({
        ...prev,
        telemetry: {
          ...prev.telemetry,
  ...
  // Called from TapperModal confirm — fires stock deduction to API
  const updateTableItems = useCallback((
    tableId: number,
    items: VenueItem[],
    mutations: { id: string; qty: number }[]
  ) => {
    ...
    // 2. Fire stock deduction to PATCH /api/products/:id for each mutated SKU
    mutations.forEach(({ id, qty }) => {
      if (id && !id.startsWith("sku_")) {
        api.patchQty(id, qty); // fire-and-forget
      }
    });
  }, []);

  // Called from LedgerCol ✕ button — deducts one unit from stock
  const removeItem = useCallback((tableId: number, itemId: string) => {
    setVenueState(prev => {
      ...
      if (productId && item) {
        // Fire stock restoration (item removed = stock goes back)
        api.patchQty(productId, Math.max(0, (item.qty ?? 1) - 1));
      }
      return {
        ...prev,
        activeTables: {
          ...prev.activeTables,
          [tableId]: { ...table, items: table.items.filter(i => i.id !== itemId) },
        },
      };
    });
  }, []);
    ...
    setVenueState(prev => ({
      ...
      telemetry: {
        ...
        bar: {
          ...prev.telemetry.bar,
          activePourSessions: Math.max(0,
            prev.telemetry.bar.activePourSessions + (delta.spiritsDepleted.length > 0 ? 1 : 0)),
        },
      },
    }));
  }, []);
  ...
  const generateOrder = useCallback(async (productId: string, qty: number, productName: string) => {
    setOrderToastMsg(`Generating purchase order: ${productName} × ${qty}...`);
    const result = await api.generateOrder(productId, qty);
    setOrderToastMsg(result
      ? `Order submitted: ${productName} × ${qty} (ID: ${result.id.slice(0,8)})`
      : `Order queued locally: ${productName} × ${qty}`
    );
    ...
    setVenueState(prev => ({
      ...
      activeTables: {
        ...
        ...(prev.activeTables[tableId] ? {
          [tableId]: { ...prev.activeTables[tableId]!, reserved: true, reservationGuest: guestName },
        } : {}),
      },
    }));
  }, []);
  ...
  // ── PROCESS PAYMENT → POST /api/pos/order ─────────────────────────────────
  ...
  return (
    <div style={{ position:"fixed", inset:0, width:"100vw", background:C.base, display:"flex", flexDirection:"column", fontFamily:C.sans, overflow:"hidden", backgroundImage:"radial-gradient(ellipse 100% 35% at 50% 0%,rgba(212,175,55,0.05),transparent 60%)" }}>
      ...
      <div style={{ flex:1, display:"grid", gridTemplateColumns:"64px 390px 1fr 480px", gridTemplateRows:"1fr", columnGap:16, padding:"16px 24px 16px 24px", overflow:"hidden", position:"relative", zIndex:1, alignItems:"stretch" }}>
        <NavRail
          onBack={back}
      ...
      {/* Purchase Order status toast */}
      <AnimatePresence>
        {orderToastMsg && (
          <motion.div key="order-toast" initial={{y:40,opacity:0}} animate={{y:0,opacity:1}} exit={{y:40,opacity:0}}
            style={{ position:"fixed", bottom:130, left:"50%", transform:"translateX(-50%)", zIndex:9300,
              background:"rgba(6,6,8,0.97)", border:`1px solid ${C.gold}`,
              borderRadius:10, padding:"10px 20px", maxWidth:480, textAlign:"center",
              boxShadow:`0 0 30px ${C.goldGlo}` }}>
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              <Icon d={P.box} size={16} color={C.gold} />
              <span style={{ fontSize:14, fontWeight:700, color:C.white }}>{orderToastMsg}</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
