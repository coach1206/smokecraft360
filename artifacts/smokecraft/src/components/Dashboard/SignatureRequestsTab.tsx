...
function RequestCard({ record, manufacturers, onUpdated }: {
  ...
  const statusColor = STATUS_COLORS[record.status] ?? "rgba(180,155,100,0.6)";
  ...
  return (
    <motion.div layout
      ...
      <button className="w-full flex items-center gap-4 p-4 text-left" onClick={() => setOpen((x) => !x)}>
        ...
        <div className="flex-shrink-0">
          {open ? <ChevronUp size={12} style={{ color: GOLD_DIM }} /> : <ChevronDown size={12} style={{ color: "rgba(180,155,100,0.35)" }} />}
        </div>
      </button>
            ...
            <div className="px-4 pb-4 space-y-4" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
              ...
              <div className="pt-3 grid grid-cols-2 gap-2 text-[8px]">
                {[
                  ...
                  <div key={label}>
                    <span style={{ color: "rgba(180,155,100,0.4)" }}>{label}: </span>
              ...
              {/* Status select */}
              <div>
                <p className="text-[8px] uppercase tracking-[0.18em] mb-1.5" style={{ color: "rgba(180,155,100,0.45)" }}>Status</p>
              ...
              {(status === "approved" || status === "production") && (
                <div>
                  <p className="text-[8px] uppercase tracking-[0.18em] mb-1.5" style={{ color: "rgba(180,155,100,0.45)" }}>Production Stage</p>
              ...
              {/* Admin notes */}
              <div>
                <p className="text-[8px] uppercase tracking-[0.18em] mb-1.5" style={{ color: "rgba(180,155,100,0.45)" }}>Admin Notes (shown to user)</p>
              ...
              <div className="flex justify-end gap-2 pt-1">
                ...
                <motion.button onClick={handleSave} disabled={saving}
                  ...
                  style={{ background: "linear-gradient(135deg, hsl(43 75% 42%), hsl(45 85% 52%))", color: "hsl(22 18% 6%)", opacity: saving ? 0.7 : 1 }}
      ...
      <div className="grid grid-cols-3 gap-2">
        {([["submitted","Pending"], ["approved","Approved"], ["production","In Production"]] as const).map(([s, label]) => (
          <div key={s} className="p-3 rounded-xl text-center"
            style={{ background: `${STATUS_COLORS[s] ?? "rgba(180,155,100,0.6)"}08`, border: `1px solid ${STATUS_COLORS[s] ?? "rgba(180,155,100,0.6)"}20` }}>
            <p className="text-xl font-serif" style={{ color: STATUS_COLORS[s], fontWeight: 300 }}>{counts[s] ?? 0}</p>
            <p className="text-[7px] uppercase tracking-[0.15em] mt-0.5" style={{ color: "rgba(180,155,100,0.38)" }}>{label}</p>
          </div>
        ))}
      </div>
