...
const CATEGORY_META: Record<string, { emoji: string; color: string; label: string }> = {
  wings:       { emoji: "🍗", color: "rgba(220,150,60,0.75)",   label: "Wings"       },
  steak:       { emoji: "🥩", color: "rgba(180,55,55,0.75)",    label: "Steak"       },
  salad:       { emoji: "🥗", color: "rgba(70,155,70,0.75)",    label: "Salad"       },
  appetizers:  { emoji: "🫙", color: "rgba(212,175,55,0.75)",   label: "Appetizers"  },
  seafood:     { emoji: "🦞", color: "rgba(65,135,195,0.75)",   label: "Seafood"     },
  desserts:    { emoji: "🍫", color: "rgba(155,75,155,0.75)",   label: "Desserts"    },
};
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {foodPairings.map((food, i) => {
          const meta = CATEGORY_META[food.category] ?? { emoji: "🍽", color: "rgba(180,155,100,0.6)", label: food.category };
          return (
            <motion.div
              key={food.id}
...
function FoodCard({
  ...
  return (
    <div
      className="relative flex flex-col p-5 rounded-xl h-full group transition-all duration-500"
      style={{
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        <span
          ...
          style={{ background: meta.color.replace("0.75", "0.12"), color: meta.color.replace("0.75", "0.85"), border: `1px solid ${meta.color.replace("0.75", "0.25")}` }}
        >
          {meta.label}
        </span>
        <span className="text-2xl leading-none" role="img" aria-label={meta.label}>{meta.emoji}</span>
      </div>
