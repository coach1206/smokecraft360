/**
 * EATTransitionOverlay — 4-stage cinematic management handoff transition.
 *
 * Stage 1  Geometric Shatter & Liquid Ripple Disruption
 *   HTML5 canvas ripple rings over the viewport + CSS rotateY(180deg) flip to #000.
 *
 ...
 *   Elastic snap-back 130% → 100% (0.4 s, cubic-bezier 0.175, 0.885, 0.32, 1.275)
 *   then gold outer neon drop-shadow emission flash locks system into command state.
 *
 * Stage 4  Fluid Workspace Deployment
 *   Logo dissolves, overlay fades, TitanEATHub command deck becomes visible.
 *
 * Trigger:  window CustomEvent "eat:enter" (fired by PosAuditBridge on admin login)
 * Resume:   Saves current path to sessionStorage("eat_guest_resume") before transition.
 */
...
// ── Constants ────────────────────────────────────────────────────────────────
const LOGO_SRC   = "/images/logo_eat.png";
const RESUME_KEY = "eat_guest_resume";

// Timing in ms from trigger
const T_VOID      = 600;    // Stage 1 complete — void black established
const T_EMERGE    = 720;    // Stage 2 begins — logo materialises
const T_SNAP      = 1520;   // Logo peaks at 130%, snap-back fires
const T_FLASH     = 1920;   // Snap settles at 100%, gold flash locks in
const T_FADE_LOGO = 2440;   // Flash holds, logo begins fade-out
const T_EXIT      = 2900;   // Overlay starts dissolving
const T_DONE      = 3200;   // Component unmounts
...
function fireBassDrop() {
  try {
    ...
    // Metallic attack shimmer: 110 Hz → 44 Hz
...
function renderRipple(canvas: HTMLCanvasElement, progress: number) {
  ...
  // Progressive black displacement curtain
...
type Stage =
  | "idle"       // not rendered
  | "ripple"     // Stage 1: canvas rings + rotateY flip
  | "void"       // pitch black void
  | "emerge"     // Stage 2: logo 0 → 130%
  | "snap"       // Stage 3: elastic 130% → 100%
  | "flash"      // Stage 3: gold emission flash
  | "fade-logo"  // Stage 4: logo fades out
  | "exit"       // Stage 4: overlay dissolves
  | "done";      // unmounted

// ── Framer Motion logo animate values per stage ──────────────────────────────
function logoAnimate(stage: Stage) {
  switch (stage) {
    ...
    case "flash":
      return { opacity: 1, scale: 1.0,  filter: "drop-shadow(0 0 35px rgba(212,175,55,0.75)) drop-shadow(0 0 72px rgba(212,175,55,0.38))" };
    case "fade-logo":
...
export function EATTransitionOverlay() {
  ...
  const activate = useCallback(() => {
    ...
    // Persist guest journey path for "Resume" button in TitanEATHub
    try {
      const cur = window.location.pathname;
      if (cur && cur !== "/titan-eat" && cur !== "/pin-login" && cur !== "/") {
        sessionStorage.setItem(RESUME_KEY, cur);
      } else if (!sessionStorage.getItem(RESUME_KEY)) {
        sessionStorage.setItem(RESUME_KEY, "/craft-hub");
      }
    } catch { /* sessionStorage unavailable */ }

    // ── Stage 1: ripple canvas ──
    setStage("ripple");
    t0Ref.current = null;
  ...
  // Listen for eat:enter custom event (fired by PosAuditBridge)
  useEffect(() => {
  ...
  return (
    ...
    >
      {/* Stage 1 — CSS perspective flip panel */}
      <AnimatePresence>
        {stage === "ripple" && (
          <motion.div
            key="flip"
      ...
      {/* Stage 1 — Canvas ripple rings + black curtain */}
      {showCanvas && (
        <canvas
          ref={canvasRef}
