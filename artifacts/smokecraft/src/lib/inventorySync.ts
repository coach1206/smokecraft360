/**
 * inventorySync.ts — Real-time inventory synchronization hook.
 *
 * Polls GET /api/products every 60s and subscribes to WebSocket
 * `inventory_updated` + `pos:ORDER_PLACED` events for instant updates.
 * Returns current items, low-stock alerts, loading state, and sync status.
 */
...
function byCategory(items: InventoryProduct[]): Record<string, InventoryProduct[]> {
...
export function useInventorySync(venueId?: string): InventorySyncState {
  const [state, setState] = useState<InventorySyncState>({
    items: [], lowStock: [], byCategory: {}, loading: true, lastUpdated: null,
    connected: false, error: null,
  });
  const timerRef = useRef<ReturnType<typeof setInterval> | null>(null);

  function applyItems(raw: unknown[]): void {
    const items = parseItems(raw);
    setState(prev => ({
      ...prev,
      items,
      lowStock:    items.filter(i => i.lowStock),
  ...
  async function fetchProducts(): Promise<void> {
    try {
      ...
      if (Array.isArray(data)) applyItems(data);
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const onOrder     = (ev: { venueId?: string; lineItems?: Array<{ name: string; qty: number }> }) => {
      ...
      setState(prev => {
        ...
        return { ...prev, items, lowStock: items.filter(i => i.lowStock), byCategory: byCategory(items) };
      });
    };
