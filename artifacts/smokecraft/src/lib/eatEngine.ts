/**
 * eatEngine.ts — Environment · Asset · Transaction foundation engine.
 *
 * Manages real-time venue operational state across three domains:
 *   ENVIRONMENT  temperature, humidity, HVAC, lighting, scent
 *   ASSET        floor tables, seating positions, spatial layout
 *   TRANSACTION  Stripe checkout, cart state, POS sync
 *
 * Connects to the API server via REST polling + WebSocket push.
 * Polling fallback fires when socket is disconnected.
 * All subscriptions use an observer pattern — components never poll directly.
 */
...
class EATEngineClass {
  ...
  async setEnvironmentMode(presetId: string): Promise<void> {
    ...
    this._applyEnv({
      ...this._env,
      activeSceneId: presetId,
      lightingMode:  preset.lighting,
      lastUpdated:   new Date().toISOString(),
    });
    try {
  ...
  async checkout(req: CheckoutRequest): Promise<CheckoutResult> {
    const res = await fetch("/api/checkout", {
      ...
      body:    JSON.stringify({
        ...
        successUrl: req.successUrl ?? `${window.location.origin}/craft-hub`,
        cancelUrl:  req.cancelUrl  ?? `${window.location.origin}/transaction`,
      }),
    });
    ...
    return {
      ...
      total:       data.total     ?? req.items.reduce((s, i) => s + i.price * i.qty, 0),
      status:      "created",
    };
  }
  ...
  private async _fetchInventory(): Promise<void> {
    try {
      ...
      const items: InventoryProduct[] = raw.map(p => ({
        id:       String(p["id"] ?? ""),
        name:     String(p["name"] ?? ""),
  ...
  private _applyFloor(tables: FloorTable[]): void {
    ...
    this._floorSubs.forEach(cb => cb(newFloor));
  }
}
