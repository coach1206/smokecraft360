 ...
 * POST /api/recommend
 *
 * Returns ranked product recommendations and cross-category pairings.
 *
 * Body:
 *  {
...
router.post("/recommend", (req: Request, res: Response) => {
  ...
  if (!Array.isArray(flavorPreferences) || flavorPreferences.length === 0) {
    res.status(400).json({
      error: '"flavorPreferences" must be a non-empty array of strings',
    });
    return;
  }
  ...
  if (typeof strength !== "number" || strength < 1 || strength > 5) {
    res.status(400).json({
      error: '"strength" must be a number between 1 and 5',
    });
    return;
  }
