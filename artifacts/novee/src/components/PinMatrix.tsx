/**
 * PinMatrix.tsx — Multi-tier PIN entry component (NOVEE OS).
 * Mirrors SmokeCraft PinMatrix exactly.
 */
...
function KeyBtn({ label, onPress, disabled = false, accent = GOLD }: {
  ...
  return (
    <motion.button
      ...
      style={{
        ...
        borderRadius: 10,
        background: pressed ? "rgba(212,175,55,0.22)" : disabled ? "rgba(255,255,255,0.03)" : "rgba(212,175,55,0.07)",
        cursor: disabled ? "default" : "pointer",
        display: "flex", alignItems: "center", justifyContent: "center",
        fontSize: 28, fontWeight: 700,
        color: disabled ? "rgba(255,255,255,0.15)" : CREAM,
        fontFamily: "'Inter', sans-serif", letterSpacing: "-0.02em",
        boxShadow: pressed ? `0 0 18px ${accent}33` : "none",
...
export function PinMatrix({
  tier: initialTier, onSuccess, onCancel, onTierChange,
  showTierSelector = false, compact = false,
}: PinMatrixProps) {
  ...
  useEffect(() => {
    ...
    if (ms > 0) {
      lockRef.current = setInterval(() => {
        const r = getLockMs(tier);
        setLockMs(r);
        if (r <= 0 && lockRef.current) { clearInterval(lockRef.current); lockRef.current = null; }
      }, 1000);
    }
  ...
  const changeTier = useCallback((t: PinTier) => {
    ...
    setLockMs(getLockMs(t)); onTierChange?.(t);
  }, [onTierChange]);
  ...
  const pushDigit = useCallback((d: string) => {
    ...
    setDigits(prev => {
      ...
      const next = [...prev, d];
      if (next.length === maxLen) void submit(next.join(""));
      return next;
    });
    setError(null);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [loading, success, lockMs, maxLen]);

  const backspace = useCallback(() => {
    if (loading || success) return;
    setDigits(p => p.slice(0, -1)); setError(null);
  }, [loading, success]);
  ...
  async function submit(pin: string) {
    ...
    if (result.locked) { setLockMs(result.lockMs ?? 0); setError(`Locked — ${fmtLock(result.lockMs ?? 0)}`); }
  ...
  return (
    <motion.div
      ...
      style={{
        background: PANEL, border: `1px solid ${label.color}33`, borderRadius: compact ? 12 : 16,
        padding: compact ? "24px 20px" : "36px 28px", width: compact ? 320 : 380, maxWidth: "100%",
        boxShadow: `0 0 60px rgba(0,0,0,0.9), 0 0 30px ${label.color}18`,
        backdropFilter: "blur(24px)", display: "flex", flexDirection: "column", gap: 20, userSelect: "none",
      }}
    >
      {showTierSelector && (
        <div style={{ display: "flex", gap: 6 }}>
          {(["staff", "management", "sovereign"] as PinTier[]).map(t => (
            <button key={t} type="button" onPointerDown={() => changeTier(t)} style={{
          ...
          return (
            <motion.div key={i}
              animate={{
                ...
                background: success ? label.color : filled ? CREAM : "rgba(255,255,255,0.10)",
              }}
      ...
      <AnimatePresence>
        {(error || loading) && (
          <motion.div key="msg"
            ...
            style={{ textAlign: "center", fontSize: 11, color: error ? "#F07070" : DIM, fontFamily: "'Inter', sans-serif", letterSpacing: "0.10em", fontWeight: 600, minHeight: 18 }}
          >
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
        {KEYS.map((k, i) => {
          ...
          if (k === "⌫") return <KeyBtn key={k} label="⌫" onPress={backspace} disabled={digits.length === 0 || loading || success || lockMs > 0} accent={AMBER} />;
          return <KeyBtn key={k} label={k} onPress={() => pushDigit(k)} disabled={loading || success || lockMs > 0} accent={label.color} />;
        })}
      </div>
      ...
      <AnimatePresence>
        {success && (
          <motion.div key="ok" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
            style={{ textAlign: "center", padding: "10px 0", color: label.color, fontSize: 13, fontWeight: 800, letterSpacing: "0.20em", textTransform: "uppercase", fontFamily: "'Inter', sans-serif" }}
          >
            ACCESS GRANTED
          </motion.div>
        )}
      ...
      {onCancel && (
        <button type="button" onPointerDown={onCancel} style={{
          ...
          padding: "10px 0", cursor: "pointer", color: DIM, fontSize: 11, fontWeight: 700,
...
export function PinMatrixOverlay({ tier, onSuccess, onCancel, showTierSelector }: PinMatrixProps) {
  return (
    <motion.div
    ...
    >
      <div style={{ position: "absolute", inset: 0, pointerEvents: "none", background: `radial-gradient(ellipse 40% 40% at 50% 50%, ${PIN_LABELS[tier].color}12 0%, transparent 70%)` }} />
      <PinMatrix tier={tier} onSuccess={onSuccess} onCancel={onCancel} showTierSelector={showTierSelector} />
    </motion.div>
  );
}
