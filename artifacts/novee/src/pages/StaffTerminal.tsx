 ...
 * Full venueState engine: revenue hooks, hardware thresholds, 15-min sync, zero-loss mutations
 */
...
const C = {
  ...
  orange: "#E67E22", sans: "'Inter','SF Pro Display',sans-serif",
...
// Zone → background image map
const ZONE_BG: Record<string, string> = {
  "VIP Section":  I("scenes/smokecraft-card.jpg"),
  "Main Floor":   I("scenes/pourcraft-card.jpg"),
  "Main Lounge":  I("scenes/bold.jpg"),
  "Outdoor":      I("scenes/relaxed.jpg"),
};
const zoneBg = (zone: string) => ZONE_BG[zone] ?? I("scenes/craft-hub.jpg");

// Item SKU → thumbnail image
const SKU_IMG: Record<string, string> = {
  sku_rp_1992:  I("cigar1.png"),
  sku_bt_bourb: I("pour/pour_whiskey.png"),
  sku_rm_xo:    I("pour/pour_aged.png"),
  sku_p64:      I("cigar2.png"),
  sku_mac12:    I("pour/pour_whiskey.png"),
  sku_af_gr:    I("cigar3.png"),
};
...
const INITIAL_STATE: VenueState = {
  ...
  activeTables: {
    101: {
      id: 101, guest: "John D.", zone: "VIP Section",
      timeStarted: new Date(Date.now() - 6_120_000).toISOString(),
      items: [
        { id: "sku_rp_1992",  name: "Rocky Patel Vintage 1992", qty: 1, price: 42.00, category: "Cigars",  img: I("cigar1.png") },
        { id: "sku_bt_bourb", name: "Buffalo Trace Bourbon",     qty: 2, price: 32.00, category: "Spirits", img: I("pour/pour_whiskey.png") },
        { id: "sku_rm_xo",    name: "Remy Martin XO",           qty: 1, price: 48.00, category: "Spirits", img: I("pour/pour_aged.png") },
        { id: "sku_ap_water", name: "Acqua Panna",               qty: 2, price:  8.00, category: "Water" },
      ],
    },
    102: {
      id: 102, guest: "Maria S.", zone: "Main Floor",
      timeStarted: new Date(Date.now() - 3_420_000).toISOString(),
      items: [
        { id: "sku_p64",   name: "Padron 1964 Anniversary",  qty: 1, price: 48.00, category: "Cigars",  img: I("cigar2.png") },
        { id: "sku_mac12", name: "Macallan 12yr Scotch",      qty: 2, price: 28.00, category: "Spirits", img: I("pour/pour_whiskey.png") },
      ],
    },
    103: {
      id: 103, guest: "Robert K.", zone: "Main Lounge",
      timeStarted: new Date(Date.now() - 1_380_000).toISOString(),
      items: [
        { id: "sku_af_gr", name: "Arturo Fuente Gran Reserva", qty: 2, price: 26.00, category: "Cigars", img: I("cigar3.png") },
      ],
    },
  },
  telemetry: {
...
function useRevenueEngine(state: VenueState) {
  return useMemo(() => {
    const tableRevenue = (tableId: number) => {
      const t = state.activeTables[tableId];
    ...
    const topCategory = (() => {
      ...
      tables.forEach(t => t.items.forEach(i => { counts[i.category] = (counts[i.category] ?? 0) + i.price * i.qty; }));
...
// ── Hardware Threshold Monitor Hook ──────────────────────────────────────────
function useThresholdMonitor(telemetry: VenueState["telemetry"]) {
  return useMemo(() => {
    const h = telemetry.humidor;
    const humidorAlert = Math.abs(h.humidity - h.targetHumidity) >= h.alertMargin || h.purosRemaining < 50;
    const humidorCritical = h.purosRemaining < 20 || Math.abs(h.humidity - h.targetHumidity) >= h.alertMargin * 2;
    const barAlert = telemetry.bar.lowStockAlerts.some(a => a.currentVolumePct < 20);
    const kitchenAlert = telemetry.kitchen.pendingOrders > 5;
    return { humidorAlert, humidorCritical, barAlert, kitchenAlert };
  }, [telemetry]);
}
...
// ── Header ────────────────────────────────────────────────────────────────────
function Header({ lastSync }: { lastSync: Date }) {
  const { time, date } = useClock();
  const [pulse, setPulse] = useState(true);
  useEffect(() => { const id = setInterval(() => setPulse(p => !p), 1100); return () => clearInterval(id); }, []);
  const syncAge = Math.floor((Date.now() - lastSync.getTime()) / 60_000);
  return (
    <div style={{ height: 50, flexShrink: 0, background: C.dark, borderBottom: `1px solid ${C.chrome}`, display: "flex", alignItems: "center", padding: "0 16px", gap: 12 }}>
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 20 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <span style={{ fontSize: 8, color: C.muted, marginLeft: 4 }}>+{syncAge}m ago</span>
        </div>
      </div>
...
function NavRail({ onBack }: { onBack: () => void }) {
  const items = [
    { icon: PATHS.house,    sub: "Hub",            active: true,  fn: onBack },
    { icon: PATHS.leaf,     sub: "SC\nSmoke\nCraft", active: false, fn: undefined },
    { icon: PATHS.cocktail, sub: "PR\nPairing",    active: false, fn: undefined },
    { icon: PATHS.utensils, sub: "CH\nCoach",      active: false, fn: undefined },
  ];
  return (
...
function TelemetryCol({
  telemetry, thresholds, onKitchenReady,
}: {
  telemetry: VenueState["telemetry"];
  thresholds: ReturnType<typeof useThresholdMonitor>;
  onKitchenReady: () => void;
}) {
  const h = telemetry.humidor;
  const b = telemetry.bar;
  const k = telemetry.kitchen;

  const metricBox = (val: string, label: string, icon: string) => (
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 7, height: "100%", overflow: "hidden" }}>
      ...
      <div style={panel({ borderTop: `2px solid ${C.gold}`, flexShrink: 0 })}>
        <div style={{ display: "flex", alignItems: "center", gap: 7, padding: "8px 12px 7px", borderBottom: `1px solid ${C.chrome}` }}>
          <Icon d={PATHS.leaf} size={14} color={C.gold} />
        ...
        <div style={{ padding: "8px 12px", borderTop: `1px solid ${C.chrome}` }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
            {metricBox(`${h.temperature}°F`, "TEMPERATURE", PATHS.thermo)}
            {metricBox(`${h.humidity}%`, "RELATIVE HUMIDITY", PATHS.drop)}
          </div>
        </div>
        <AnimatePresence>
          {thresholds.humidorAlert && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: [1, 0.65, 1] }} exit={{ opacity: 0 }} transition={{ duration: 1.3, repeat: Infinity }}
              style={{ display: "flex", alignItems: "center", gap: 8, background: thresholds.humidorCritical ? "rgba(192,57,43,0.35)" : C.redLo, borderTop: `1px solid ${C.red}44`, padding: "8px 12px" }}>
              <Icon d={PATHS.warn} size={13} color={C.redHi} />
              <span style={{ flex: 1, fontSize: 11, fontWeight: 800, color: C.redHi, letterSpacing: "0.14em" }}>
                {thresholds.humidorCritical ? "CRITICAL: RESTOCK NOW" : "TARGET ALERT: LOW STOCK"}
              </span>
      ...
      <div style={panel({ borderTop: "2px solid #3A6BC4", flexShrink: 0 })}>
        <div style={{ display: "flex", alignItems: "center", gap: 7, padding: "8px 12px 7px", borderBottom: `1px solid ${C.chrome}` }}>
          <Icon d={PATHS.cocktail} size={14} color="#5B8DEF" />
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr auto", alignItems: "center", padding: "10px 12px", gap: 10 }}>
          ...
          <div style={{ width: 48, height: 72, borderRadius: 7, background: `url(${I("pour/pour_whiskey.png")}) center/cover no-repeat,#1A0A00`, border: `1px solid ${C.chrome}` }} />
        </div>
        {b.lowStockAlerts.map(a => (
          <div key={a.item} style={{ display: "flex", alignItems: "center", gap: 8, background: C.redLo, borderTop: `1px solid ${C.red}44`, padding: "8px 12px" }}>
            <Icon d={PATHS.warn} size={13} color={C.redHi} />
            <div>
              <div style={{ fontSize: 10, fontWeight: 800, color: C.redHi, letterSpacing: "0.16em" }}>LOW STOCK — {a.currentVolumePct}%</div>
      ...
      <div style={panel({ borderTop: "2px solid #7B5EA7", flex: 1, display: "flex", flexDirection: "column" })}>
        <div style={{ display: "flex", alignItems: "center", gap: 7, padding: "8px 12px 7px", borderBottom: `1px solid ${C.chrome}` }}>
          <Icon d={PATHS.utensils} size={14} color="#A78BFA" />
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, padding: "10px 12px" }}>
          {[
            { val: k.pendingOrders, label: "PENDING ORDERS", color: C.amber },
            { val: k.readyOrders,   label: "READY ORDERS",   color: C.green },
          ].map(m => (
        ...
        <motion.button whileTap={{ scale: 0.97 }} {...T} onClick={onKitchenReady}
          style={{ marginTop: "auto", width: "100%", height: 42, background: "rgba(123,94,167,0.10)", border: "none", borderTop: "1px solid rgba(123,94,167,0.28)", color: "#A78BFA", fontSize: 12, fontWeight: 800, letterSpacing: "0.14em", cursor: "pointer", fontFamily: C.sans, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
          <Icon d={PATHS.list} size={14} color="#A78BFA" />
          VIEW KITCHEN QUEUE
      ...
      <motion.div initial={{ scale: 0.94, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.94, y: 20 }}
        ...
        <div style={{ padding: "14px 20px", borderBottom: `1px solid ${C.chrome}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <motion.button whileTap={{ scale: 0.9 }} onClick={onClose}
            style={{ width: 32, height: 32, borderRadius: 8, background: "rgba(255,255,255,0.05)", border: `1px solid ${C.chrome}`, color: C.muted, cursor: "pointer", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center" }}>
            ×
          </motion.button>
        </div>
              ...
              <motion.div key={item.id} layout initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10, height: 0 }}
                ...
                {([["−", -1, C.redHi], ["+", 1, C.green]] as const).map(([l, d, c]) => (
                  <motion.button key={String(l)} whileTap={{ scale: 0.88 }} onClick={() => adj(item.id, d)}
                    style={{ width: 30, height: 30, borderRadius: "50%", background: `${c}1a`, border: `1px solid ${c}44`, color: c, fontSize: 18, lineHeight: "28px", textAlign: "center", cursor: "pointer" }}>
                    {l}
                  </motion.button>
                ))}
...
function TableCard({ table, isActive, onSelect, onTap }: {
  ...
  const tab = table.items.reduce((s, i) => s + i.price * i.qty, 0);
      ...
      <div style={{ display: "grid", gridTemplateColumns: "38% 62%", height: 118 }}>
        ...
        <div style={{ padding: "10px 12px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 22, fontWeight: 900, color: C.amber, lineHeight: 1.1 }}>${tab.toFixed(2)}</div>
          </div>
        </div>
      </div>
...
function TicketsCol({ state, revenue, onSelect, onUpdate }: {
  ...
  onUpdate: (tableId: number, items: VenueItem[]) => void;
}) {
  const [filter, setFilter] = useState<FilterTab>("ALL TABLES");
  const [tapTable, setTapTable] = useState<VenueTable | null>(null);

  const allTables = Object.values(state.activeTables);
  const filtered = filter === "ALL TABLES" ? allTables
    : allTables.filter(t => t.zone.toUpperCase().includes(filter.replace(" SECTION", "").replace(" FLOOR", "").replace(" LOUNGE", "")));
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", gap: 5, marginBottom: 8, flexShrink: 0, alignItems: "center" }}>
        {FILTER_TABS.map(t => (
          <motion.button key={t} whileTap={{ scale: 0.95 }} onClick={() => setFilter(t)}
            style={{ height: 26, padding: "0 9px", borderRadius: 5, cursor: "pointer", background: filter === t ? `linear-gradient(135deg,${C.gold},#A67C00)` : "rgba(255,255,255,0.04)", border: `1px solid ${filter === t ? C.gold : C.chrome}`, color: filter === t ? "#000" : C.muted, fontSize: 9, fontWeight: 800, letterSpacing: "0.12em", fontFamily: C.sans, whiteSpace: "nowrap" }}>
            {t}
          </motion.button>
        ))}
        <div style={{ marginLeft: "auto" }}><Icon d={PATHS.filter} size={16} color={C.muted} /></div>
      </div>

      <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: 7 }}>
        {filtered.map(t => (
          <TableCard key={t.id} table={t}
            isActive={t.id === state.selectedTableId}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", background: C.glass2, border: `1px solid ${C.chrome}`, borderRadius: 7, marginTop: 7, flexShrink: 0 }}>
        {[
          { l: "ACTIVE TABLES",   v: String(allTables.length) },
          { l: "VIP TABLES",      v: String(allTables.filter(t => t.zone.toLowerCase().includes("vip")).length) },
      ...
      <AnimatePresence>
        {tapTable && (
          <TapperModal key={tapTable.id} table={tapTable}
            ...
            onUpdate={items => { onUpdate(tapTable.id, items); setTapTable(null); }} />
        )}
      </AnimatePresence>
    </div>
  );
...
function LedgerCol({ state, revenue, onRemoveItem }: {
  state: VenueState; revenue: ReturnType<typeof useRevenueEngine>;
        ...
        <AnimatePresence>
          ...
          {table.items.map((item, idx) => (
            <motion.div key={item.id} layout initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10, height: 0 }}
              ...
              <motion.button whileTap={{ scale: 0.84 }} {...T} onClick={() => onRemoveItem(table.id, item.id)}
...
function Footer({ revenue }: { revenue: ReturnType<typeof useRevenueEngine> }) {
  return (
    <div style={{ flexShrink: 0, height: 116, background: C.dark, borderTop: `1px solid ${C.chrome}`, display: "grid", gridTemplateColumns: "1fr 1fr 1fr" }}>
          ...
          {STAFF.map(s => (
            <div key={s.name} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
              <div style={{ position: "relative" }}>
                <div style={{ width: 36, height: 36, borderRadius: "50%", background: "linear-gradient(135deg,#3D2B1F,#1A120A)", border: `2px solid ${C.chrome}`, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
      ...
      <div style={{ padding: "10px 16px" }}>
        <div style={{ fontSize: 10, fontFamily: C.mono, color: C.gold, letterSpacing: "0.26em", marginBottom: 8 }}>SYSTEM STATUS</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 6 }}>
          {SYS_STATUS.map(s => (
            <div key={s.label} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
              <div style={{ width: 34, height: 34, borderRadius: 7, background: `${s.color}1a`, border: `1px solid ${s.color}44`, display: "flex", alignItems: "center", justifyContent: "center" }}>
...
export default function StaffTerminal({ onBack: onBackProp }: { onBack?: () => void } = {}) {
      ...
      setVenueState(prev => ({
        ...
        telemetry: {
          ...
          humidor: {
            ...
            // Simulate natural humidity drift ±1%
            humidity: Math.max(60, Math.min(80,
              prev.telemetry.humidor.humidity + (Math.random() > 0.5 ? 1 : -1))),
            // Slow puro depletion
            purosRemaining: Math.max(0, prev.telemetry.humidor.purosRemaining - Math.floor(Math.random() * 2)),
          },
          bar: {
            ...prev.telemetry.bar,
            activePourSessions: Math.max(0,
              prev.telemetry.bar.activePourSessions + Math.floor(Math.random() * 5 - 2)),
          },
        },
      }));
    }, ms);
    return () => clearInterval(id);
  }, [venueState.syncIntervalMinutes]);
  ...
  const updateTableItems = useCallback((tableId: number, items: VenueItem[]) => {
    setVenueState(prev => ({
      ...prev,
      activeTables: {
        ...prev.activeTables,
        [tableId]: { ...prev.activeTables[tableId], items },
      },
    }));
  }, []);
  ...
  const removeItem = useCallback((tableId: number, itemId: string) => {
    setVenueState(prev => ({
      ...
      activeTables: {
        ...prev.activeTables,
        [tableId]: {
          ...prev.activeTables[tableId],
          items: prev.activeTables[tableId].items.filter(i => i.id !== itemId),
        },
      },
    }));
  }, []);
  ...
  const thresholds = useThresholdMonitor(venueState.telemetry);
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: C.base, display: "flex", flexDirection: "column", fontFamily: C.sans, overflow: "hidden", backgroundImage: "radial-gradient(ellipse 100% 35% at 50% 0%,rgba(212,175,55,0.05),transparent 60%)" }}>
      ...
      <div style={{ position: "relative", zIndex: 2 }}>
        <Header lastSync={venueState.lastSyncAt} />
      </div>
