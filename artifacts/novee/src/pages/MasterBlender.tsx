...
import {
  GOLD, MASTERY_TIERS, type MasteryTier, getTier,
  COUNTRY_FLAGS, persistCountry, loadVisitedCountries,
  SENSORY_MATRIX, HALO_R, HALO_CIRC,
  LEAVES, WRAPPERS, VITOLAS, CUTS, STEP_MENTOR,
  MENTORS, SEEDS, SOILS,
  type GatewayPhase, type XPFloat, type Sel, type PairingResult,
} from "@/lib/masterBlenderData";
...
function AlchemyReveal({
  ...
  const { profile: guestProfile } = useGuest();
  const isReturning = !!guestProfile?.firstName;
  ...
  const flavorHistory: { tag: string; count: number }[] = [];
...
export default function MasterBlender() {
  const { setPhase: _setPhase } = useGuest();
  const { speak, stopSpeak } = useAudio();
  const { profile: guestProfile, addPoints: _addPoints } = useGuest();
      ...
      {gateway === "cockpit" && (
        <div style={{ position: "fixed", inset: 0, background: "#000000", zIndex: 99998, display: "flex", flexDirection: "column" }}>
          <CockpitIdleView onCraft={(id) => {
            ...
            else if (id === "pour") { _setPhase("crafthub"); }
            else if (id === "beer") { _setPhase("crafthub"); }
            else if (id === "wine") { _setPhase("crafthub"); }
          }} />
        </div>
      )}
